...
Context API was created to make state sharing down a component tree easier. It also prevent prop drilling.

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

Actions are functions that perform specific processed that change stored state data, using the reducer, which dispacted those actions.The store is where state data is stored.

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state affects the entire application, whereas component state affects only individual components. If the components need to be reused, the component state would be better.

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-thunk is  a middleware that lets you call actions that will return a function instead of an object.

5. What is your favorite state management system you've learned and this sprint? Please explain why!

Redux, because I think it is going to be the most valuable to me in my career.
